...
def save_contours_to_svg(contours, output_svg_path, width, height,
    ...
    for contour_idx, contour in enumerate(contours):
        ...
        points_str += " Z"
        svg_paths.append(f'<path d="{points_str}" stroke="{stroke_color}" stroke-width="{stroke_width}" fill="{fill_color}" />')

    if not svg_paths:
        print(f"SVG_SAVE_INFO: No valid path strings generated from contours for '{output_svg_path}'. SVG will not be created.")
        return False
...
def main():
    ...
    if svg_was_saved:
        ...
        print(f"         Common issues: incorrect Canny/blur parameters, or all contours filtered by area/approximation.")
    print("---------------------------")
